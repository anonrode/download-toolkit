...
def _ensure_package(import_name, pip_name=None):
    """
    Try to import a package. If missing, install it automatically via pip
    and retry the import. Returns the imported module or None on failure.
    """
    import importlib
    ...
    try:
        ...
        try:
            subprocess.run(
                ['pip', 'install', pip_name, '--break-system-packages', '-q'],
                check=True, stdin=subprocess.DEVNULL,
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
            return importlib.import_module(import_name)
        except Exception as e:
            print(f'  [!] Auto-install failed for {pip_name}: {e}')
            return None

# Auto-install core dependencies if missing before importing them
_ensure_package('requests')
_ensure_package('bs4', 'beautifulsoup4')
...
def resolve_wildshare(url):
    if not _ensure_package('curl_cffi', 'curl_cffi'):
        safe_print("  [!] curl_cffi could not be installed — Wildshare unavailable")
        return None
    try:
...
def _extract_downloadwella_site(url, session, ctx, site_label, name_cleaner):
    ...
    # Split links into batches of `parallel` size
    # For each batch: resolve all links simultaneously, then download simultaneously
    ...
    for batch in batches:
        ...
        # Filter out already-downloaded episodes in this batch
        to_process = []
        for ep_url in batch:
            ep_index += 1
        ...
        if len(to_process) == 1 or batch_size == 1:
            ...
            # Multiple episodes — resolve all in parallel, then download in parallel
            from concurrent.futures import ThreadPoolExecutor, as_completed

            # Step 1: Resolve all links in batch simultaneously
            ...
            # Step 2: Download all resolved links in parallel
            items = []
