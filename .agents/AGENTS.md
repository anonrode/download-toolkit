...
5. **This file is authoritative and permanent. Do not delete, rename, or move it, and do not replace it with a task-scoped scratch doc.** If you track your own work-in-progress notes, keep them in a *separate* file (e.g. `PROJECT.md`) — never at the expense of this charter. Update §10/§11 in place as state changes; never blank the file. It is the single source of truth every agent (including future-you) depends on to avoid re-breaking what's already been fixed.
   ...
   *Concrete proof this matters — three live bugs shipped in commits because §0.2/§0.3 were skipped (all from the same agent, same audit):*
   ...
   - `_asearch_anitaku` called `BeautifulSoup(...)` but **the name was never imported** in `search.py`. Every call raised `NameError`, swallowed by its own `try/except`, so `ani <title>` reported "no result" forever while the `?s=` fetch returned 200 with full HTML. This is §0.3 in the flesh: a silent `NameError` inside an `except` that looks exactly like a dead site. It slipped a first review because the reviewer tested a *reimplementation* that imported bs4 itself — **you must run the REAL code path (`_run_search_async` here), not a lookalike.** A `python -c "import ast; ast.parse(...)"` or grep for every bare name against the module's imports would have caught it before shipping.

   **Meta-lesson: this agent (Antigravity/Gemini) writes plausible, well-structured code fast but repeatedly codes against assumptions instead of the live world and does not run the happy path end to end. Treat its commits as needs-review, not trust-and-merge — especially anything touching selectors, endpoints, imports, or JSON fields. Audit every batch.**
6. **Preserve the resumability contract.** A user can Ctrl-C, lose Wi-Fi, or close the app at any instant. Nothing you do may turn "interrupted" into "lost work" or "silently skipped." The three-bucket resume state (§5) is load-bearing. Don't route around it.
...
**anitaku.com.ro (Gogoanime) search — use the AJAX JSON API, not the `?s=` HTML.** `_asearch_anitaku` posts to `https://anitaku.com.ro/wp-admin/admin-ajax.php` with `action=ts_ac_do_search&ts_ac_query=<term>` (send `X-Requested-With: XMLHttpRequest` + a `Referer`). It returns `{"series":[{"all":[{post_title, post_type, post_status, post_sub, post_latest, post_link, ID, ...}]}]}` — the same badge data the old scrape dug out of `div.listupd article.bs` selectors, delivered first-party, so it's immune to layout churn and the sidebar-pollution/soft-404 traps. Label contract unchanged: `"Anitaku (anime): {title}\x00{type · status · sub}"`. **The theme has no episode-pager AJAX action** (read all `dramastream*` JS — only `ts_ac_do_search`, bookmark, and view-counter actions exist). **Critical catalog fact: `post_latest` is a *label* (the ep number the show has aired to), NOT how many episodes anitaku hosts.** Old episodes 301-redirect to the homepage — for One Piece, `episode-1/2/500/1000` all bounce; only the latest ~35 window (e.g. 1137–1171) resolves. So there is nothing to paginate to, and a range picker can only offer the hosted window. Don't build episode pagination against this site.
