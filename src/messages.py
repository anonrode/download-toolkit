...
MESSAGES = {
    ...
    'update_dirty': ('warn', 'You have unpushed commits — push them before updating so they are not lost:'),
    'update_local_changes': ('warn', 'An update is available ({count} commit(s) behind), but these tracked files were changed locally:'),
    'update_cancelled': ('info', 'Update cancelled. Nothing was changed.'),
    ...
    'hls_resuming_partial': ('info', 'Resuming from where it stopped (keeping downloaded fragments)...'),
    'ytdlp_frag_recover': ('warn', 'Fragment {frag} was unavailable - clearing it and retrying ({attempt}/{total})...'),
    'ytdlp_frag_gave_up': ('fail', 'Fragment {frag} still unavailable after {total} attempts - the stream has a hole.'),
